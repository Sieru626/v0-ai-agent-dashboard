...
function InventoryCard({ item }: { item: InventoryItem }) {
  ...
  return (
    <div
      className={`p-2 border ${rarity.border} bg-[#0d0d0d] hover:bg-[#111] transition-all cursor-pointer group`}
    >
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1">
            <span className="text-[7px] text-[#ccc] truncate block flex-1">
              {item.name}
            </span>
            <span className={`text-[6px] ${rarity.text} font-bold`}>
              {rarity.label}
            </span>
          </div>
      ...
      <div className="p-2 border-t border-[#1a1a1a]">
        ...
        <div className="flex gap-1 mt-1.5">
          {Object.entries(rarityColors).map(([key, val]) => (
            <div
              key={key}
              className={`flex-1 text-center py-0.5 border ${val.border} text-[5px] ${val.text}`}
            >
              {val.label}
            </div>
          ))}
